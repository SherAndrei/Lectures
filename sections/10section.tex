...
\begin{definition}
    ...
    \(\Theta_0\in\Theta\), называется \defin{параметрической гипотезой}.
...
\begin{definition}
    ...
    \defin{простой}.
    В противном случае \(H_0(H_1)\) - \defin{сложная}.
\end{definition}
...
\begin{definition}
    Множество \(S\) называется \defin{критическим множеством} или \defin{критерием},
    \(\overline{S}\) - \defin{область принятия гипотезы}.
\end{definition}
\begin{definition}
...
\begin{remark}
    ...
    называется \defin{критерием отношения правдоподобия Неймана-Пирсона}.
\end{remark}
\begin{remark}
    ...
    Это свойство называется \defin{несмещенностью критерия} \(S(\lambda)\)
\end{remark}
\begin{proof}
